...
class SourcedArticle:
    ...
    def store_article(self):
        logger.info(f"Storing article {self.article_id}")
        ...
        store_object_in_s3(
            SOURCED_ARTICLES_S3_BUCKET, article_key, self.raw_article.json(), self.s3_client
        )
        ...
        store_object_in_s3(
            SOURCED_ARTICLES_S3_BUCKET, long_summary_key, self.long_article_summary, self.s3_client
        )
        store_success_file(
            SOURCED_ARTICLES_S3_BUCKET, article_dir_prefix, self.success_marker_fn, self.s3_client
        )
    ...
    def _summarize_article(self, summarization_length: SummarizationLength) -> str:
        ...
        with get_openai_callback() as cb:
            ...
            if summary == SUMMARIZATION_FAILURE_MESSAGE:
                raise ArticleSummarizationFailure(self.article_id)
            # TODO - emit metrics
            logger.info(
